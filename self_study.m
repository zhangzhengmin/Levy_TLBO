%
% Project Title: Levy_TLBO in MATLAB
%
...
function [best_result, teacher] = self_study(teacher,searching_times,len,job_info,job_position,machine_number,job_time,best_result)
teacher_copy=teacher;
teacher_pop=cell(searching_times,1);
teacher_res=zeros(searching_times,1);
selfstudy_factor=ceil(abs(normrnd(1,1)/(normrnd(1,4))^1/1.5))*ceil(len/80);
if selfstudy_factor>len
    selfstudy_factor=len;
end
